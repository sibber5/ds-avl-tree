<<<<<<< HEAD
#id1:213856032
#name2:Nara Zangariya
#username1:naraz
=======
#id1:
#name1:
#username1:
>>>>>>> fdb61f64ac4d8dc5d90f66edcdab32d0ee762503
#id2:
#name2:
#username2:
...
<<<<<<< HEAD
    ...
    """
    def search(self, key):
        return self.search_rec(self.root, 1, key)

    def search_rec(self,node,e,k):
        if node is None:
            return None,e
        if node.key==k:
            return node,e
        if node.key>k:
            return self.search_rec(node.left, e + 1, k)
        if node.key<k:
            return self.search_rec(node.right, e + 1, k)
